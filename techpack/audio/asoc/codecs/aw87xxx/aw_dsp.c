			...
			void *data, int data_size)
{
	...
	real_len = data_size + sizeof(mtk_dsp_hdr_t);
					...
					int data_size)
{
	...
	ret = mtk_spk_recv_ipi_buf_from_dsp(data, data_size, &real_len);
	if (ret < 0)
		goto failed;
...
EXPORT_SYMBOL(aw_check_dsp_ready);
